...
/*
*** Statistical inference of large SqrtPGM with cross validation
***
...
double log_factorial_SqrtPGM_CV(double m){
...
NumericVector log_factorial_vector_SqrtPGM_CV(NumericVector m){
	size_t n = m.size();
...
NumericVector P1_calculation_SqrtPGM_CV(NumericVector predict, double diag_interact){
	...
	do{
		...
		numerator = numerator + sqrt(j)*exp(sqrt(j)*predict + diag_interact*j - log_factorial_SqrtPGM_CV(j));
		denominator = denominator + exp(sqrt(j)*predict + diag_interact*j - log_factorial_SqrtPGM_CV(j));
...
NumericVector P2_calculation_SqrtPGM_CV(NumericVector predict, double diag_interact){
	size_t n = predict.size();
	NumericVector numerator(n);
	NumericVector denominator(n);
	NumericVector p2(n);
	...
	do{
		...
		numerator = numerator + j*exp(sqrt(j)*predict + diag_interact*j - log_factorial_SqrtPGM_CV(j));
		denominator = denominator + exp(sqrt(j)*predict + diag_interact*j - log_factorial_SqrtPGM_CV(j));
		NumericVector diff1 = numerator - numerator_tmp;
		NumericVector diff2 = denominator - denominator_tmp;
		if((max(abs(diff1)) <= 1e-2) && (max(abs(diff2)) <= 1e-2)){
...
NumericVector log_term_SqrtPGM_CV(NumericVector predict, double diag_interact){
	size_t n = predict.size();
	NumericVector log_term_SqrtPGM_CV(n);
	NumericVector log_term_SqrtPGM_CV_tmp(n);

	double j = 0;
	do{
		for(size_t i = 0; i < n; i++){
			log_term_SqrtPGM_CV_tmp[i] = log_term_SqrtPGM_CV[i];
		}
		log_term_SqrtPGM_CV = log_term_SqrtPGM_CV + exp(sqrt(j)*predict + diag_interact*j - log_factorial_SqrtPGM_CV(j));
		NumericVector diff = log_term_SqrtPGM_CV - log_term_SqrtPGM_CV_tmp;
	...
	return log_term_SqrtPGM_CV;
}

/* ****** Sub-function, fast Lasso regression for SqrtPGM with intercept ******* */
List FastSqrtPGMLasso_Rcpp_SqrtPGM_CV(NumericVector beta, NumericVector predictor, NumericVector p1, NumericVector p2, NumericVector ipy, NumericVector y, NumericMatrix x, double lambda, double step_size, size_t N){
  ...
  //NumericVector p1 = P1_calculation_SqrtPGM_CV(predictor, beta[0]);
  //NumericVector p2 = P2_calculation_SqrtPGM_CV(predictor, beta[0]);
  ...
  do{
	...
	double current_g = -sum(y*predictor + beta[0]*pow(y,2) - log_factorial_vector_SqrtPGM_CV(pow(y,2)) - log(log_term_SqrtPGM_CV(predictor,beta[0])))/N;
    ...
    do{
    	...
    	g_new = -sum(y*predictor_update + x_plus[0]*pow(y,2) - log_factorial_vector_SqrtPGM_CV(pow(y,2)) - log(log_term_SqrtPGM_CV(predictor_update,x_plus[0])))/N;
    ...
    p1 = P1_calculation_SqrtPGM_CV(predictor, beta[0]);
    p2 = P2_calculation_SqrtPGM_CV(predictor, beta[0]);
...
List FastSqrtPGMLasso_Rcpp2_SqrtPGM_CV(NumericVector beta, NumericVector predictor, NumericVector p1, NumericVector p2, NumericVector ipy, NumericVector y, NumericMatrix x, double lambda, double step_size, size_t N){
	  double tol = 1e-3; //threshold for stopping
	  size_t p = ipy.size();
	  double dbm;  //maximum of beta difference for while loop
	  //NumericVector beta(p+1); //initialize output beta vector with length p and filled with 0.0
...
void shufflesample_SqrtPGM_CV(IntegerVector sample_index, size_t n){
	while(n >= 1 ){
		size_t k = rand() % n;
		n = n-1;
...
/* ***** Sub-function, k-fold ***** */
IntegerVector k_fold_SqrtPGM_CV(size_t n, size_t kfold){
	 IntegerVector indices(n);
	 for(size_t i = 0; i < n; i++){
		 indices[i] = i % kfold;
	 }
     shufflesample_SqrtPGM_CV(indices, n);
     return indices;
}
...
double min_position_SqrtPGM_CV(NumericVector x){
...
NumericVector sortDescending_SqrtPGM_CV(NumericVector x){
...
/* ***** Function 1 ******** */
// [[Rcpp::export]]
List FastSqrtPGM_CV_FDR(NumericMatrix x, double kfold = NA_REAL, double nlambda = NA_REAL, double step_size = NA_REAL, bool intercept=true, bool global=false, Nullable<NumericVector> regularization=R_NilValue, Nullable<NumericVector> alpha=R_NilValue, double N=NA_REAL, double delta_upper=NA_REAL, Nullable<NumericMatrix> true_graph=R_NilValue){
...
if(regularization.isNull()){
	  ...
	  //NumericMatrix supnorm(100,p);
		  ...
		  for(size_t i = 1; i < p+1; i++){
			  ...
			  IntegerVector cv_index = k_fold_SqrtPGM_CV(n, kfold);
		 ...
		 NumericVector sqrt_IP_YX_new_train(p+1);
		 ...
		 for(int fold = 0; fold < kfold; fold++){
		   ...
		   for(size_t sample = 0; sample < n; sample++){
		     if(cv_index[sample] == fold){
		       testingset(t1,_) = sqrt_x_new(sample,_);
		       t1++;
		     }
		     else{
		       trainingset(t2,_) = sqrt_x_new(sample,_);
		       t2++;
		     }
		   }

		   for(size_t b = 0; b < p+1; b++){
		     sqrt_IP_YX_new_train[b] = inner_product(trainingset(_,i).begin(), trainingset(_,i).end(), trainingset(_,b).begin(), 0.0);
		   }

		   //Extract IP_YX_new_train[-i], trainingset[-i]
		   NumericVector sqrt_tmpyx_train(p);
		   NumericMatrix sqrt_tmp_x_train(n - record,p);
		   NumericMatrix sqrt_tmp_x_test(record,p);
		   size_t t = 0;
		   for (size_t k = 0; k < p+1; k++){
		     if (k != i){
		       sqrt_tmpyx_train[t] = sqrt_IP_YX_new_train[k];
		       sqrt_tmp_x_train(_,t) = trainingset(_,k);
		       sqrt_tmp_x_test(_,t) = testingset(_,k);
		       t++;
		     }
		   }
		   ...
		   NumericVector p1_train = P1_calculation_SqrtPGM_CV(predictor_train, beta_tmp[0]);
		   NumericVector p2_train = P2_calculation_SqrtPGM_CV(predictor_train, beta_tmp[0]);

		   for (size_t k = 0; k < nlambda; k++){
		     List Lasso_fit = FastSqrtPGMLasso_Rcpp_SqrtPGM_CV(beta_tmp, predictor_train, p1_train, p2_train, sqrt_tmpyx_train, trainingset(_,i), sqrt_tmp_x_train, lambda[k], step_size, n-record);
		     ...
		     for(size_t b = 0; b < record; b++){
		       predictor_test[b] = inner_product(sqrt_tmp_x_test(b,_).begin(), sqrt_tmp_x_test(b,_).end(), beta_tmp.begin(), 0.0);
		     }

		     NumericVector log_likelihood_test = testingset(_,i)*predictor_test + beta_tmp[0]*pow(testingset(_,i),2) - log_factorial_vector_SqrtPGM_CV(pow(testingset(_,i),2)) - log(log_term_SqrtPGM_CV(predictor_test, beta_tmp[0]));
		 ...
		 double position = min_position_SqrtPGM_CV(avg_CV_criterion);
		 ...
		 List Lasso_fit = FastSqrtPGMLasso_Rcpp_SqrtPGM_CV(beta, predictor, p1, p2, sqrt_tmpyx, sqrt_x_new(_,i), sqrt_tmp_x, lambda[position], step_size, n);
		 beta = Lasso_fit["beta"];
	 ...
	 for (size_t i = 0; i < p-1; i++){
			    ...
			    NumericVector psi1 = P1_calculation_SqrtPGM_CV(tau, theta_initial(i,i));
			    NumericVector psi2 = P2_calculation_SqrtPGM_CV(tau, theta_initial(i,i)) - pow(psi1,2);
	            ...
	            do{
	            	   ...
	            	   do{
	            	           for(size_t index1 = 0; index1 < n; index1++){
	            	            ...
	            	            capital_m = capital_m + exp(phi[i]*sqrt(k1)+theta_initial(i,i)*k1-log_factorial_SqrtPGM_CV(k1)+phi[j]*sqrt(k2)+theta_initial(j,j)*k2-log_factorial_SqrtPGM_CV(k2)+theta_initial(i,j)*sqrt(k1)*sqrt(k2)+sqrt(k1)*m1+sqrt(k2)*m2);
	            	   ...
	            	   NumericVector inter_step = (P2_calculation_SqrtPGM_CV(sqrt(k2)*theta_initial(i,j)+m1+phi[i],theta_initial(i,i))-pow(P1_calculation_SqrtPGM_CV(sqrt(k2)*theta_initial(i,j)+m1+phi[i],theta_initial(i,i)),2))*capital_m;
	      ...
	      if(NumericVector::is_na(kfold)){
			  ...
			  //NumericMatrix supnorm(100,p);
				 ...
				 NumericVector sqrt_IP_YX_train(p);
				 NumericMatrix CV_criterion(kfold, nlambda);
				 ...
				 for(int fold = 0; fold < kfold; fold++){
				   ...
				   for(size_t sample = 0; sample < n; sample++){
				     if(cv_index[sample] == fold){
				       testingset(t1,_) = sqrt_x(sample,_);
				       t1++;
				     }
				     else{
				       trainingset(t2,_) = sqrt_x(sample,_);
				       t2++;
				     }
				   }

				   for(size_t b = 0; b < p; b++){
				     sqrt_IP_YX_train[b] = inner_product(trainingset(_,i).begin(), trainingset(_,i).end(), trainingset(_,b).begin(), 0.0);
				   }

				   //Extract IP_YX_train[-i], trainingset[-i]
				   NumericVector sqrt_tmpyx_train(p-1);
				   NumericMatrix sqrt_tmp_x_train(n - record,p-1);
				   NumericMatrix sqrt_tmp_x_test(record,p-1);
				   size_t t = 0;
				   for (size_t k = 0; k < p; k++){
				     if (k != i){
				       sqrt_tmpyx_train[t] = sqrt_IP_YX_train[k];
				       sqrt_tmp_x_train(_,t) = trainingset(_,k);
				       sqrt_tmp_x_test(_,t) = testingset(_,k);
				       t++;
				     }
				   ...
				   for (size_t k = 0; k < nlambda; k++){
				     List Lasso_fit = FastSqrtPGMLasso_Rcpp2_SqrtPGM_CV(beta_tmp, predictor_train, p1_train, p2_train, sqrt_tmpyx_train, trainingset(_,i), sqrt_tmp_x_train, lambda[k], step_size, n-record);
				     beta_tmp = Lasso_fit["beta"];
				     predictor_train = Lasso_fit["predictor"];
				     p1_train = Lasso_fit["p1"];
				     p2_train = Lasso_fit["p2"];
				 ...
				 List Lasso_fit = FastSqrtPGMLasso_Rcpp2_SqrtPGM_CV(beta, predictor, p1, p2, sqrt_tmpyx, sqrt_x(_,i), sqrt_tmp_x, lambda[position], step_size, n);
				 beta = Lasso_fit["beta"];
			    	...
			    	for(size_t j = i+1; j < p; j++){
			            ...
			            do{
			            	 ...
			            	 do{
			            	        ...
			            	        capital_m = capital_m + exp(theta_initial(i,i)*k1-log_factorial_SqrtPGM_CV(k1)+theta_initial(j,j)*k2-log_factorial_SqrtPGM_CV(k2)+theta_initial(i,j)*sqrt(k1)*sqrt(k2)+sqrt(k1)*m1+sqrt(k2)*m2);
			            	        NumericVector diff = capital_m - capital_m_tmp;
			            	        if(max(abs(diff)) <= 1e-2){
			            	            	 break;
			            	        }
			            	 ...
			            	 NumericVector inter_step = (P2_calculation_SqrtPGM_CV(sqrt(k2)*theta_initial(i,j)+m1,theta_initial(i,i))-pow(P1_calculation_SqrtPGM_CV(sqrt(k2)*theta_initial(i,j)+m1,theta_initial(i,i)),2))*capital_m;
			            	 g_numerator = g_numerator + sqrt(k2)*inter_step;
			            	 g_denominator = g_denominator + inter_step;
	  ...
	  return List::create(_["intercept"] = phi, _["theta_initial"] = theta_initial, _["theta_cor"] = theta_cor, _["CI_low_theta"] = CI_low_theta, _["CI_high_theta"] = CI_high_theta, _["z_score"] = test_statistic, _["p_thetaCor"] = p_thetaCor, _["est_sd"] = est_stand_deviation);
		...
		if(intercept==true){
           ...
           p2 = Lasso_fit["p2"];
			...
			delta = sortDescending_SqrtPGM_CV(delta);
		   ...
		   for(size_t i = 0; i < p; i++){
					tmp_p1(_,i) = P1_calculation_SqrtPGM_CV(tmp_predictor(_,i), tmp_beta(0,i));
		   }
		   NumericMatrix tmp_p2(n,p);
		   for(size_t i = 0; i < p; i++){
				    tmp_p2(_,i) = P2_calculation_SqrtPGM_CV(tmp_predictor(_,i), tmp_beta(0,i));
		   }
		   ...
		   for(size_t delta_count = 0; delta_count < ndelta; delta_count++){
			   ...
			   for(size_t i = 1; i < p+1; i++){
					...
					NumericVector p2_column = tmp_p2(_,i-1);
				  	...
				  	List beta_result = FastSqrtPGMLasso_Rcpp_SqrtPGM_CV(beta_column, predictor_column, p1_column, p2_column, sqrt_tmpyx, sqrt_x_new(_,i), sqrt_tmp_x, lambda, step_size, n);
			   ...
			   Rcout << "Bias correction for initial estimators. " << endl;
			   	 ...
			   	 for (size_t i = 0; i < p-1; i++){
			   	  ...
			   	  error_cum[delta_count] = error_1;
		   				...
		   				double position_select = min_position_SqrtPGM_CV(error_cum);
			...
			for(size_t delta_count = 0; delta_count < ndelta; delta_count++){
					...
					for(size_t i = 0; i < p; i++){
						...
						List beta_result = FastSqrtPGMLasso_Rcpp2_SqrtPGM_CV(beta_column, predictor_column, p1_column, p2_column, sqrt_tmpyx, sqrt_x(_,i), sqrt_tmp_x, lambda, step_size, n);
	...
	rcpp_regularization = sortDescending_SqrtPGM_CV(rcpp_regularization);
