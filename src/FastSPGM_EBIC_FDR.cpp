...
/*
*** Statistical inference of large SPGM with EBIC
***
...
NumericVector sublinear_SPGM_EBIC(NumericVector x, NumericVector lower, NumericVector upper){
	size_t p = x.size();
	NumericVector y(p);
...
double sublinear2_SPGM_EBIC(double x, double lower, double upper){
	double y;
	if(x <= lower){
		y = x;
	}
...
double log_factorial_SPGM_EBIC(double m){
...
NumericVector log_factorial_vector_SPGM_EBIC(NumericVector m){
	size_t n = m.size();
...
NumericVector log_term_SPGM_EBIC(NumericVector predict, double threshold1, double threshold2){
	size_t n = predict.size();
	NumericVector log_term_SPGM_EBIC(n);
	NumericVector log_term_SPGM_EBIC_tmp(n);

	double j = 0;
	do{
		for(size_t i = 0; i < n; i++){
			log_term_SPGM_EBIC_tmp[i] = log_term_SPGM_EBIC[i];
		}
		log_term_SPGM_EBIC = log_term_SPGM_EBIC + exp(sublinear2_SPGM_EBIC(j,threshold1,threshold2)*predict - log_factorial_SPGM_EBIC(j));
		NumericVector diff = log_term_SPGM_EBIC - log_term_SPGM_EBIC_tmp;
	...
	return log_term_SPGM_EBIC;
}

/* ****** Sub-function, P1 calculation ****** */
NumericVector P1_calculation_SPGM_EBIC(NumericVector predict, double threshold1, double threshold2){
	size_t n = predict.size();
	NumericVector p1(n);
	...
	do{
		...
		numerator = numerator + sublinear2_SPGM_EBIC(j,threshold1,threshold2)*exp(sublinear2_SPGM_EBIC(j,threshold1,threshold2)*predict-log_factorial_SPGM_EBIC(j));
		denominator = denominator + exp(sublinear2_SPGM_EBIC(j,threshold1,threshold2)*predict-log_factorial_SPGM_EBIC(j));
...
NumericVector P2_calculation_SPGM_EBIC(NumericVector predict, double threshold1, double threshold2){
	size_t n = predict.size();
	NumericVector p2(n);
	NumericVector numerator(n);
	NumericVector denominator(n);
	...
	do{
		...
		numerator = numerator + pow(sublinear2_SPGM_EBIC(j,threshold1,threshold2),2)*exp(sublinear2_SPGM_EBIC(j,threshold1,threshold2)*predict-log_factorial_SPGM_EBIC(j));
		denominator = denominator + exp(sublinear2_SPGM_EBIC(j,threshold1,threshold2)*predict-log_factorial_SPGM_EBIC(j));
		NumericVector diff1 = numerator - numerator_tmp;
		NumericVector diff2 = denominator - denominator_tmp;
		if((max(abs(diff1)) <= 1e-2) && (max(abs(diff2)) <= 1e-2)){
...
List FastSPGMLasso_Rcpp(NumericVector beta, NumericVector predictor, NumericVector p1, NumericVector p2, NumericVector ipy, NumericVector y, NumericMatrix x, double lambda, double step_size, double threshold1, double threshold2, size_t N){
  ...
  //NumericVector p1 = P1_calculation_SPGM_EBIC(predictor, threshold1, threshold2);
  ...
  do{
	...
	double current_g = -sum(y*predictor - log(log_term_SPGM_EBIC(predictor,threshold1,threshold2)))/N;
    ...
    do{
    	...
    	g_new = -sum(y*predictor_update - log(log_term_SPGM_EBIC(predictor_update,threshold1,threshold2)))/N;
    ...
    p1 = P1_calculation_SPGM_EBIC(predictor, threshold1, threshold2);
    p2 = P2_calculation_SPGM_EBIC(predictor, threshold1, threshold2);
...
List FastSPGMLasso_Rcpp2(NumericVector beta, NumericVector predictor, NumericVector p1, NumericVector p2, NumericVector ipy, NumericVector y, NumericMatrix x, double lambda, double step_size, double threshold1, double threshold2, size_t N){
	double tol = 1e-3; //threshold for stopping
	  size_t p = ipy.size();
	  double dbm;  //maximum of beta difference for while loop
	  //NumericVector beta(p); //initialize output beta vector with length p and filled with 0.0
...
double min_position_SPGM_EBIC(NumericVector x){
...
NumericVector sortDescending_SPGM_EBIC(NumericVector x){
...
/* ***** Function 1 ******** */
// [[Rcpp::export]]
List FastSPGM_EBIC_FDR(NumericMatrix x, NumericVector D_0, NumericVector D_1, double gamma = NA_REAL, double nlambda = NA_REAL, double step_size = NA_REAL, bool intercept=true, bool global=false, Nullable<NumericVector> regularization=R_NilValue, Nullable<NumericVector> alpha=R_NilValue, double N=NA_REAL, double delta_upper=NA_REAL, Nullable<NumericMatrix> true_graph=R_NilValue){
...
if(regularization.isNull()){
	  ...
	  if(intercept == true){
		  ...
		  for(size_t i = 0; i < n; i++){
			  sub_x(i,_) = sublinear_SPGM_EBIC(x(i,_), D_0, D_1);
		  }
	  ...
	  //NumericMatrix supnorm(100,p);
		 ...
		 NumericVector predictor(n);
		 NumericVector p1 = P1_calculation_SPGM_EBIC(predictor, D_0[i-1], D_1[i-1]);
		 NumericVector p2 = P2_calculation_SPGM_EBIC(predictor, D_0[i-1], D_1[i-1]);
		 //NumericVector weight = p2 - pow(p1,2);

		 for (size_t k = 0; k < nlambda; k++){

			 	 //List Lasso_fit = FastSPGMLasso_Rcpp(beta, predictor, p1, p2, weight, sub_tmpyx, sub_tmp_x, sub_tmp_xx, lambda[k], D_0[i-1], D_1[i-1], n);
			 	 List Lasso_fit = FastSPGMLasso_Rcpp(beta, predictor, p1, p2, sub_tmpyx, sub_x_new(_,i), sub_tmp_x, lambda[k], step_size, D_0[i-1], D_1[i-1], n);
			 	 beta = Lasso_fit["beta"];
		 		 ...
		 		 NumericVector log_likelihood = sub_x_new(_,i)*predictor - log_factorial_vector_SPGM_EBIC(x(_,i-1)) - log(log_term_SPGM_EBIC(predictor, D_0[i-1], D_1[i-1]));
		 		 BIC_value[k] = -2*sum(log_likelihood) + count*(log(n)+2*gamma*log(p-1));
		 }


		 // Return the position of the minimum BIC
		 double position = min_position_SPGM_EBIC(BIC_value);
	 ...
	 for (size_t i = 0; i < p-1; i++){
			    ...
			    NumericVector psi1 = P1_calculation_SPGM_EBIC(tau, D_0[i], D_1[i]);
			    NumericVector psi2 = P2_calculation_SPGM_EBIC(tau, D_0[i], D_1[i]) - pow(psi1,2);
	    	...
	    	for(size_t j = i+1; j < p; j++){
	            ...
	            //for(size_t k2 = 0; k2 <= D[j]; k2++){
		            ...
		            //for(size_t k1 = 0; k1 <= D[i]; k1++){
	            		...
	            		capital_m = capital_m + exp(phi[i]*sublinear2_SPGM_EBIC(k1,D_0[i],D_1[i])-log_factorial_SPGM_EBIC(k1)+phi[j]*sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])-log_factorial_SPGM_EBIC(k2)+theta_initial(i,j)*sublinear2_SPGM_EBIC(k1,D_0[i],D_1[i])*sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])+sublinear2_SPGM_EBIC(k1,D_0[i],D_1[i])*m1+sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*m2);
	            	...
	            	NumericVector inter_step = (P2_calculation_SPGM_EBIC(sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*theta_initial(i,j)+m1+phi[i],D_0[i],D_1[i])-pow(P1_calculation_SPGM_EBIC(sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*theta_initial(i,j)+m1+phi[i],D_0[i],D_1[i]),2))*capital_m;
	            	g_numerator = g_numerator + sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*inter_step;
			    ...
			    //NumericMatrix supnorm(100,p);
				 ...
				 NumericVector predictor(n);
				 NumericVector p1 = P1_calculation_SPGM_EBIC(predictor, D_0[i], D_1[i]);
				 NumericVector p2 = P2_calculation_SPGM_EBIC(predictor, D_0[i], D_1[i]);
				 //NumericVector weight = p2 - pow(p1,2);

				 for (size_t k = 0; k < nlambda; k++){

				 	 	 //List Lasso_fit = FastSPGMLasso_Rcpp2(beta, predictor, p1, p2, weight, sub_tmpyx, sub_tmp_x, sub_tmp_xx, lambda[k], D_0[i-1], D_1[i-1], n);
				 	 	 List Lasso_fit = FastSPGMLasso_Rcpp2(beta, predictor, p1, p2, sub_tmpyx, sub_x(_,i), sub_tmp_x, lambda[k], step_size, D_0[i], D_1[i], n);
					 	 beta = Lasso_fit["beta"];
				 		 predictor = Lasso_fit["predictor"];
				 		 p1 = Lasso_fit["p1"];
				 		 p2 = Lasso_fit["p2"];
				 		 ...
				 		 NumericVector log_likelihood = sub_x(_,i)*predictor - log_factorial_vector_SPGM_EBIC(x(_,i)) - log(log_term_SPGM_EBIC(predictor, D_0[i], D_1[i]));
				 		 BIC_value[k] = -2*sum(log_likelihood) + count*(log(n)+2*gamma*log(p-1));
				 }
			    	...
			    	for(size_t j = i+1; j < p; j++){
			            ...
			            do{
			            	...
			            	//for(size_t k1 = 0; k1 <= D[i]; k1++){
			            	    ...
			            	    capital_m = capital_m + exp(-log_factorial_SPGM_EBIC(k1)-log_factorial_SPGM_EBIC(k2)+theta_initial(i,j)*sublinear2_SPGM_EBIC(k1,D_0[i],D_1[i])*sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])+sublinear2_SPGM_EBIC(k1,D_0[i],D_1[i])*m1+sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*m2);
			            	    NumericVector diff = capital_m - capital_m_tmp;
			            	    if(max(abs(diff)) <= 1e-2){
			            	    	break;
			            	    }
			            	...
			            	NumericVector inter_step = (P2_calculation_SPGM_EBIC(sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*theta_initial(i,j)+m1,D_0[i],D_1[i])-pow(P1_calculation_SPGM_EBIC(sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*theta_initial(i,j)+m1,D_0[i],D_1[i]),2))*capital_m;
			            	g_numerator = g_numerator + sublinear2_SPGM_EBIC(k2,D_0[j],D_1[j])*inter_step;
			            	g_denominator = g_denominator + inter_step;
			...
			// Sublinear transformation on the original data matrix x
				  ...
				  //NumericMatrix supnorm(100,p);
					 ...
					 noise[i-1] = variance_record[position];
					  ...
					  delta = sortDescending_SPGM_EBIC(delta);
					  ...
					  for(size_t i = 0; i < p; i++){
					  		tmp_p1(_,i) = P1_calculation_SPGM_EBIC(tmp_predictor(_,i), D_0[i], D_1[i]);
					  }
					  NumericMatrix tmp_p2(n,p);
					  for(size_t i = 0; i < p; i++){
					  		tmp_p2(_,i) = P2_calculation_SPGM_EBIC(tmp_predictor(_,i), D_0[i], D_1[i]);
					  }
					  ...
					  for(size_t delta_count = 0; delta_count < ndelta; delta_count++){
						  ...
						  for(size_t i = 1; i < p+1; i++){
						  			...
						  			//List beta_result = FastSPGMLasso_Rcpp(beta_column, predictor_column, p1_column, p2_column, weight_column, sub_tmpyx, sub_tmp_x, sub_tmp_xx, lambda, D_0[i-1], D_1[i-1], n);
								 	List beta_result = FastSPGMLasso_Rcpp(beta_column, predictor_column, p1_column, p2_column, sub_tmpyx, sub_x_new(_,i), sub_tmp_x, lambda, step_size, D_0[i-1], D_1[i-1], n);
							 ...
							 error_cum[delta_count] = error_1;
					  				  ...
					  				  double position_select = min_position_SPGM_EBIC(error_cum);
				...
				for(size_t delta_count = 0; delta_count < ndelta; delta_count++){
					...
					for(size_t i = 0; i < p; i++){
							...
							//List beta_result = FastSPGMLasso_Rcpp2(beta_column, predictor_column, p1_column, p2_column, weight_column, sub_tmpyx, sub_tmp_x, sub_tmp_xx, lambda, D_0[i], D_1[i], n);
					 	 	List beta_result = FastSPGMLasso_Rcpp2(beta_column, predictor_column, p1_column, p2_column, sub_tmpyx, sub_x(_,i), sub_tmp_x, lambda, step_size, D_0[i], D_1[i], n);
	...
	rcpp_regularization = sortDescending_SPGM_EBIC(rcpp_regularization);
