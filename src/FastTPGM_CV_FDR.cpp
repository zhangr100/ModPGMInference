...
/*
*** Statistical inference of large TPGM with cross validation
***
By Rong Zhang, 2020.09.13
*/

/* ****** Sub-function, log factorial ****** */
double log_factorial_TPGM_CV(double m){
...
NumericVector log_factorial_vector_TPGM_CV(NumericVector m){
	size_t n = m.size();
...
NumericVector log_term_TPGM_CV(NumericVector predict, double threshold){
	size_t n = predict.size();
	NumericVector log_term_TPGM_CV(n);
	//NumericVector log_term_TPGM_CV_tmp(n);

	for(double j = 0; j <= threshold; j++){
		log_term_TPGM_CV = log_term_TPGM_CV + exp(j*predict - log_factorial_TPGM_CV(j));
	}

	return log_term_TPGM_CV;
}

/* ****** Sub-function, P1 calculation ****** */
NumericVector P1_calculation_TPGM_CV(NumericVector predict, double threshold){
	size_t n = predict.size();
	NumericVector p1(n);
	...
	//for(size_t i = 0; i < n; i++){
		for(double j = 0; j <= threshold; j++){
			numerator = numerator + j*exp(j*predict-log_factorial_TPGM_CV(j));
			denominator = denominator + exp(j*predict-log_factorial_TPGM_CV(j));
		}
...
NumericVector P2_calculation_TPGM_CV(NumericVector predict, double threshold){
	size_t n = predict.size();
	NumericVector p2(n);
	NumericVector numerator(n);
	NumericVector denominator(n);
	//for(size_t i = 0; i < n; i++){
		for(double j = 0; j <= threshold; j++){
			numerator = numerator + pow(j,2)*exp(j*predict-log_factorial_TPGM_CV(j));
			denominator = denominator + exp(j*predict-log_factorial_TPGM_CV(j));
		}
...
List FastTPGMLasso_Rcpp_TPGM_CV(NumericVector beta, NumericVector predictor, NumericVector p1, NumericVector p2, NumericVector ipy, NumericVector y, NumericMatrix x, double lambda, double step_size, double thresh, size_t N){
  ...
  //NumericVector p1 = P1_calculation_TPGM_CV(predictor, threshold1, threshold2);
  ...
  do{
	...
	double current_g = -sum(y*predictor - log(log_term_TPGM_CV(predictor,thresh)))/N;
    ...
    do{
    	...
    	g_new = -sum(y*predictor_update - log(log_term_TPGM_CV(predictor_update,thresh)))/N;
    ...
    p1 = P1_calculation_TPGM_CV(predictor, thresh);
    p2 = P2_calculation_TPGM_CV(predictor, thresh);
...
List FastTPGMLasso_Rcpp2_TPGM_CV(NumericVector beta, NumericVector predictor, NumericVector p1, NumericVector p2, NumericVector ipy, NumericVector y, NumericMatrix x, double lambda, double step_size, double thresh, size_t N){
	double tol = 1e-3; //threshold for stopping
	  size_t p = ipy.size();
	  double dbm;  //maximum of beta difference for while loop
	  //NumericVector beta(p); //initialize output beta vector with length p and filled with 0.0
...
double min_position_TPGM_CV(NumericVector x){
...
NumericVector sortDescending_TPGM_CV(NumericVector x){
	sort(x.begin(), x.end(), greater<double>());
	return x;
}

/* ***** Sub-function, shuffle sample ***** */
void shufflesample_TPGM_CV(IntegerVector sample_index, size_t n){
  while(n >= 1){
    size_t k = rand() % n;
    n = n-1;
...
/* ***** Sub-function, k-fold ***** */
IntegerVector k_fold_TPGM_CV(size_t n, size_t kfold){
  IntegerVector indices(n);
  for(size_t i = 0; i < n; i++){
    indices[i] = i % kfold;
  }
  shufflesample_TPGM_CV(indices, n);
  return indices;
}
...
/* ***** Function 1 ******** */
// [[Rcpp::export]]
List FastTPGM_CV_FDR(NumericMatrix x, NumericVector D, double kfold = NA_REAL, double nlambda = NA_REAL, double step_size = NA_REAL, bool intercept=true, bool global=false, Nullable<NumericVector> alpha=R_NilValue, Nullable<NumericVector> regularization=R_NilValue, double N=NA_REAL, double delta_upper=NA_REAL, Nullable<NumericMatrix> true_graph=R_NilValue){
  ...
  if(regularization.isNull()){
	  ...
	  //NumericMatrix supnorm(100,p);
		  ...
		  for(size_t i = 1; i < p+1; i++){
			  ...
			  IntegerVector cv_index = k_fold_TPGM_CV(n, kfold);
		 ...
		 NumericVector IP_YX_new_train(p+1);
		 ...
		 for(int fold = 0; fold < kfold; fold++){
		   ...
		   for(size_t sample = 0; sample < n; sample++){
		      if(cv_index[sample] == fold){
		        testingset(t1,_) = x_new(sample,_);
		        t1++;
		      }
		      else{
		        trainingset(t2,_) = x_new(sample,_);
		        t2++;
		      }
		   }

	     for(size_t b = 0; b < p+1; b++){
	       IP_YX_new_train[b] = inner_product(trainingset(_,i).begin(), trainingset(_,i).end(), trainingset(_,b).begin(), 0.0);
	     }

	     //Extract IP_YX_new_train[-i], trainingset[-i]
	     NumericVector tmpyx_train(p);
		   NumericMatrix tmp_x_train(n - record,p);
		   NumericMatrix tmp_x_test(record,p);
		   size_t t = 0;
		   for (size_t k = 0; k < p+1; k++){
		     if (k != i){
		       tmpyx_train[t] = IP_YX_new_train[k];
		       tmp_x_train(_,t) = trainingset(_,k);
		       tmp_x_test(_,t) = testingset(_,k);
		       t++;
		     }
		   }
		   ...
		   NumericVector p1_train = P1_calculation_TPGM_CV(predictor_train, D[i-1]);
		   NumericVector p2_train = P2_calculation_TPGM_CV(predictor_train, D[i-1]);

		   for(size_t k = 0; k < nlambda; k++){
		     List Lasso_fit = FastTPGMLasso_Rcpp_TPGM_CV(beta_tmp, predictor_train, p1_train, p2_train, tmpyx_train, trainingset(_,i), tmp_x_train, lambda[k], step_size, D[i-1], n-record);
		     ...
		     for(size_t b = 0; b < record; b++){
		       predictor_test[b] = inner_product(tmp_x_test(b,_).begin(), tmp_x_test(b,_).end(), beta_tmp.begin(), 0.0);
		     }

		    NumericVector exp_term_test(record);
		    for(double j = 0; j <= D[i-1]; j++){
		       exp_term_test = exp_term_test + exp(j*predictor_test-log_factorial_TPGM_CV(j));
		    }

		    NumericVector log_likelihood_test = testingset(_,i)*predictor_test - log_factorial_vector_TPGM_CV(testingset(_,i)) - log(exp_term_test);
		 ...
		 double position = min_position_TPGM_CV(avg_CV_criterion);

		 NumericVector beta(p);
		 NumericVector predictor(n);
		 NumericVector p1 = P1_calculation_TPGM_CV(predictor, D[i-1]);
		 NumericVector p2 = P2_calculation_TPGM_CV(predictor, D[i-1]);
		 //NumericVector weight = p2 - pow(p1,2);

		 List Lasso_fit = FastTPGMLasso_Rcpp_TPGM_CV(beta, predictor, p1, p2, tmpyx, x_new(_,i), tmp_x, lambda[position], step_size, D[i-1], n);
		 beta = Lasso_fit["beta"];
	 ...
	 for (size_t i = 0; i < p-1; i++){
			    ...
			    NumericVector psi1 = P1_calculation_TPGM_CV(tau, D[i]);
			    NumericVector psi2 = P2_calculation_TPGM_CV(tau, D[i]) - pow(psi1,2);
	    	...
	    	for(size_t j = i+1; j < p; j++){
	            ...
	            for(size_t k2 = 0; k2 <= D[j]; k2++){
	            	...
	            	for(size_t k1 = 0; k1 <= D[i]; k1++){
	            		capital_m = capital_m + exp(phi[i]*k1-log_factorial_TPGM_CV(k1)+phi[j]*k2-log_factorial_TPGM_CV(k2)+theta_initial(i,j)*k1*k2+k1*m1+k2*m2);
	            	}
	            	NumericVector inter_step = (P2_calculation_TPGM_CV(k2*theta_initial(i,j)+m1+phi[i],D[i])-pow(P1_calculation_TPGM_CV(k2*theta_initial(i,j)+m1+phi[i],D[i]),2))*capital_m;
				  ...
				  for(size_t i = 0; i < p; i++){
					  ...
					  // Take the tuning parameters
				 		 ...
				 		 for(int fold = 0; fold < kfold; fold++){
				 		   ...
				 		   for(size_t sample = 0; sample < n; sample++){
				 		     if(cv_index[sample] == fold){
				 		       testingset(t1,_) = x(sample,_);
				 		       t1++;
				 		     }
				 		     else{
				 		       trainingset(t2,_) = x(sample,_);
				 		       t2++;
				 		     }
				 		   }

				 		   for(size_t b = 0; b < p; b++){
				 		     IP_YX_train[b] = inner_product(trainingset(_,i).begin(), trainingset(_,i).end(), trainingset(_,b).begin(), 0.0);
				 		   }

				 		   //Extract IP_YX_train[-i], trainingset[-i]
				 		   NumericVector tmpyx_train(p-1);
				 		   NumericMatrix tmp_x_train(n - record,p-1);
				 		   NumericMatrix tmp_x_test(record,p-1);
				 		   size_t t = 0;
				 		   for (size_t k = 0; k < p; k++){
				 		     if (k != i){
				 		       tmpyx_train[t] = IP_YX_train[k];
				 		       tmp_x_train(_,t) = trainingset(_,k);
				 		       tmp_x_test(_,t) = testingset(_,k);
				 		       t++;
				 		     }
				 		   ...
				 		   NumericVector predictor_train(n - record);
				 		   NumericVector predictor_test(record);
				 		   NumericVector p1_train = P1_calculation_TPGM_CV(predictor_train, D[i]);
				 		   NumericVector p2_train = P2_calculation_TPGM_CV(predictor_train, D[i]);

				 		   for(size_t k = 0; k < nlambda; k++){
				 		     List Lasso_fit = FastTPGMLasso_Rcpp2_TPGM_CV(beta_tmp, predictor_train, p1_train, p2_train, tmpyx_train, trainingset(_,i), tmp_x_train, lambda[k], step_size, D[i], n-record);
				 		     beta_tmp = Lasso_fit["beta"];
				 		     predictor_train = Lasso_fit["predictor"];
				 		     p1_train = Lasso_fit["p1"];
				 		     p2_train = Lasso_fit["p2"];
				 		 ...
				 		 NumericVector predictor(n);
				 		 NumericVector p1 = P1_calculation_TPGM_CV(predictor, D[i]);
				 		 NumericVector p2 = P2_calculation_TPGM_CV(predictor, D[i]);
				 		 //NumericVector weight = p2 - pow(p1,2);


				 		 //List Lasso_fit = FastTPGMLasso_Rcpp2_TPGM_CV(beta, predictor, p1, p2, weight, tmpyx, tmp_x, tmp_xx, lambda[k], D[i], n);
					 	 List Lasso_fit = FastTPGMLasso_Rcpp2_TPGM_CV(beta, predictor, p1, p2, tmpyx, x(_,i), tmp_x, lambda[position], step_size, D[i], n);
				 		 beta = Lasso_fit["beta"];
			 ...
			 for (size_t i = 0; i < p-1; i++){
			    	...
			    	for(size_t j = i+1; j < p; j++){
			            ...
			            for(size_t k2 = 0; k2 <= D[j]; k2++){
				            NumericVector capital_m(n);
			            	for(size_t k1 = 0; k1 <= D[i]; k1++){
			            		capital_m = capital_m + exp(-log_factorial_TPGM_CV(k1)-log_factorial_TPGM_CV(k2)+theta_initial(i,j)*k1*k2+k1*m1+k2*m2);
			            	}
			            	NumericVector inter_step = (P2_calculation_TPGM_CV(k2*theta_initial(i,j)+m1,D[i])-pow(P1_calculation_TPGM_CV(k2*theta_initial(i,j)+m1,D[i]),2))*capital_m;
			            	g_numerator = g_numerator + k2*inter_step;
			            	g_denominator = g_denominator + inter_step;
			            }
	  ...
	  return List::create(_["intercept"] = phi, _["theta_initial"] = theta_initial, _["theta_cor"] = theta_cor, _["CI_low_theta"] = CI_low_theta, _["CI_high_theta"] = CI_high_theta, _["z_score"] = test_statistic, _["p_thetaCor"] = p_thetaCor, _["est_sd"] = est_stand_deviation);
		...
		if(intercept == true){
				  ...
				  delta = sortDescending_TPGM_CV(delta);
				  ...
				  for(size_t i = 0; i < p; i++){
					  tmp_p1(_,i) = P1_calculation_TPGM_CV(tmp_predictor(_,i), D[i]);
				  }
				  NumericMatrix tmp_p2(n,p);
				  for(size_t i = 0; i < p; i++){
					  tmp_p2(_,i) = P2_calculation_TPGM_CV(tmp_predictor(_,i), D[i]);
				  }
				  ...
				  for(size_t delta_count = 0; delta_count < ndelta; delta_count++){
				  		  ...
				  		  for(size_t i = 1; i < p+1; i++){
							  ...
							  double lambda = delta[delta_count]*sqrt(noise[i-1]*(log(p)/n));
				  			 ...
				  			 //List beta_result = FastTPGMLasso_Rcpp_TPGM_CV(beta_column, predictor_column, p1_column, p2_column, weight_column, tmpyx, tmp_x, tmp_xx, lambda, D[i-1], n);
							 List beta_result = FastTPGMLasso_Rcpp_TPGM_CV(beta_column, predictor_column, p1_column, p2_column, tmpyx, x_new(_,i), tmp_x, lambda, step_size, D[i-1], n);
				  ...
				  double position_select = min_position_TPGM_CV(error_cum);
							  ...
							  for(size_t delta_count = 0; delta_count < ndelta; delta_count++){
							  		  ...
							  		  for(size_t i = 0; i < p; i++){
							  			...
							  			for (size_t k = 0; k < p; k++){
							  			 ...
							  			 //List beta_result = FastTPGMLasso_Rcpp2_TPGM_CV(beta_column, predictor_column, p1_column, p2_column, weight_column, tmpyx, tmp_x, tmp_xx, lambda, D[i], n);
									 	 List beta_result = FastTPGMLasso_Rcpp2_TPGM_CV(beta_column, predictor_column, p1_column, p2_column, tmpyx, x(_,i), tmp_x, lambda, step_size, D[i], n);
	  ...
	  rcpp_regularization = sortDescending_TPGM_CV(rcpp_regularization);
